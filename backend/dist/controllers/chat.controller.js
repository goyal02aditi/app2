...
exports.restoreChat = exports.archiveChat = exports.autoArchiveOldChats = exports.setActiveChat = exports.deleteMessage = exports.deleteChat = exports.sendMessageWithAI = exports.addMessage = exports.getChatById = exports.getMostRecentChat = exports.getUserChats = exports.startChatWithMessage = void 0;
 ...
 */
exports.startChatWithMessage = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    let autoTitle;
    try {
        autoTitle = await (0, openai_service_1.generateAIResponse)([{ role: "user", content: titlePrompt }]);
    ...
    const newChat = await chat_model_1.default.create({
    ...
    try {
        ...
        const aiResponse = await (0, openai_service_1.generateAIResponse)(conversationHistory, userId.toString());
 ...
 */
exports.getUserChats = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id; // User added by auth middleware
 ...
 */
exports.getMostRecentChat = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id;
    // Find the most recently updated chat for the user
    const recentChat = await chat_model_1.default.findOne({
        userId: new mongoose_1.Types.ObjectId(userId),
        isActive: true
    })
        .sort({ updatedAt: -1 }) // Most recent first
 ...
 */
exports.getChatById = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!mongoose_1.Types.ObjectId.isValid(chatId)) {
        throw new Apierror_1.ApiError(400, "Invalid chat ID format");
    }
    ...
    if (!chat) {
        throw new Apierror_1.ApiError(404, "Chat not found or access denied");
    }
    return res.status(200).json({
        success: true,
 ...
 */
exports.addMessage = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { chatId } = req.params;
    const { message } = req.body;
    const userId = req.user._id;
    // Validate inputs
 ...
 */
exports.sendMessageWithAI = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { chatId } = req.params;
    ...
    try {
        ...
        return res.status(200).json({
            ...
            data: {
                chat: await chat_model_1.default.findById(chatId),
                userMessage: message.trim(),
                error: "AI service temporarily unavailable"
            }
        });
 ...
 */
exports.deleteChat = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { chatId } = req.params;
    const userId = req.user._id;
    // Validate chat ID
    if (!chatId) {
    ...
    if (timeDifference > fifteenMinutesInMs) {
        throw new Apierror_1.ApiError(403, "Chat deletion not allowed. You can only delete chats within 15 minutes of creation. This chat is now permanently stored for research purposes.");
    }
    // Soft delete - set isActive to false (preserves data for research)
    chat.isActive = false;
    chat.archivedAt = new Date();
    chat.archiveReason = 'user-deleted-within-window';
    await chat.save();
    return res.status(200).json({
        success: true,
...
exports.deleteMessage = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    // Validate IDs
    if (!chatId || !messageId) {
        throw new Apierror_1.ApiError(400, "Chat ID and Message ID are required");
    }
    if (!mongoose_1.Types.ObjectId.isValid(chatId)) {
        throw new Apierror_1.ApiError(400, "Invalid chat ID format");
    }
    if (!mongoose_1.Types.ObjectId.isValid(messageId)) {
        throw new Apierror_1.ApiError(400, "Invalid message ID format");
    }
    // Find and verify chat ownership
    const chat = await chat_model_1.default.findOne({
        _id: new mongoose_1.Types.ObjectId(chatId),
    ...
    const fifteenMinutesInMs = 15 * 60 * 1000; // 15 minutes in milliseconds
    if (timeDifference > fifteenMinutesInMs) {
        throw new Apierror_1.ApiError(403, "Message deletion not allowed. You can only delete messages within 15 minutes of sending. This message is now permanently stored for research purposes.");
    }
 ...
 */
exports.setActiveChat = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { chatId } = req.params;
    const userId = req.user._id;
    // Validate chat ID
    if (!chatId) {
 ...
 */
exports.autoArchiveOldChats = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id;
    const daysOld = parseInt(req.query.days) || 30; // Default 30 days
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - daysOld);
    // Find chats that haven't been updated in X days
    const oldChats = await chat_model_1.default.updateMany({
        userId: new mongoose_1.Types.ObjectId(userId),
        isActive: true,
 ...
 */
exports.archiveChat = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { chatId } = req.params;
    ...
    if (!chat) {
        throw new Apierror_1.ApiError(404, "Chat not found or already archived");
    }
    // Archive the chat (different from delete)
    chat.isActive = false;
    chat.archivedAt = new Date();
    chat.archiveReason = reason || 'user-archived';
    await chat.save();
    return res.status(200).json({
        success: true,
 ...
 */
exports.restoreChat = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { chatId } = req.params;
    const userId = req.user._id;
    // Validate chat ID
    if (!chatId) {
    ...
    if (!chat) {
        throw new Apierror_1.ApiError(404, "Archived chat not found");
    }
    // Restore the chat
    chat.isActive = true;
